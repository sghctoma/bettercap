  ...
  class Utils
    def self.ifconfig(iface='eth0')
      ...
      if BetterCap::Shell.available?('ifconfig')
        ...
        when /freebsd/i
          ret = freebsd_ifconfig iface, data
        end
    ...
    def self.freebsd_ifconfig(iface='eth0',ifconfig_data)
      BetterCap::Logger.debug "FreeBSD ifconfig #{iface}:\n#{ifconfig_data}"
      ...
      ifconfig_data.each do |s|
        case s
          when /ether[\s]([0-9a-fA-F:]{17})/i
            ret[:eth_saddr] = $1
            ret[:eth_src] = EthHeader.mac2str(ret[:eth_saddr])
          when /inet[\s]*([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)(.*netmask[\s]+(0x[a-f0-9]+))?/i
            imask = 0
            if $3
              imask = $3.to_i(16).to_s(2).count("1")
            end
